...
export default function Adopt() {
  ...
  return (
    <View className="min-h-screen bg-secondary pt-24 pb-24 px-4 sm:px-6">
      <View className="max-w-6xl mx-auto">
        
        {/* Header & Judul */}
        <View className="text-center mb-8">
          <Text className="text-3xl font-bold text-dark mb-2">Temukan Teman Barumu</Text>
          <Text className="text-gray-500">Kucing-kucing lucu ini menunggu rumah baru.</Text>
        </div>

        {/* Filter Bar (Search & Select) */}
        <View className="bg-white p-4 rounded-2xl shadow-sm mb-8 flex flex-col md:flex-row gap-4 items-center border border-gray-100">
          
          {/* Input Pencarian */}
          <View className="relative w-full md:w-2/3">
          ...
          {/* Dropdown Ras */}
          <View className="relative w-full md:w-1/3">
        ...
        {loading && (
          <View className="text-center py-20">
            <View className="animate-spin rounded-full h-12 w-12 border-b-2 border-primary mx-auto mb-4"></div>
            <Text className="text-gray-500">Sedang memanggil kucing...</Text>
          </div>
        )}
          ...
          <View className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-4 md:gap-6">
            {filteredCats.length > 0 ? (
              filteredCats.map((cat) => (
                <View key={cat.id} className="bg-white rounded-2xl shadow-sm hover:shadow-md transition-shadow overflow-hidden group border border-gray-100">
                  
                  {/* Foto Kucing */}
                  <View className="relative aspect-[4/5] overflow-hidden bg-gray-100">
                    <img 
                      src={cat.image} 
                      alt={cat.name} 
                      className="w-full h-full object-cover group-hover:scale-105 transition-transform duration-500"
                    />
                    <View className="absolute top-2 right-2 bg-white/80 backdrop-blur-sm px-2 py-1 rounded-lg text-xs font-bold text-primary shadow-sm">
                      {cat.age} Thn
                    </div>
                  </div>
                  ...
                  <View className="p-4">
                    ...
                    <Text className="text-gray-500 text-sm mb-3">{cat.breed || 'Campuran'}</Text>
              ...
              <View className="col-span-full text-center py-10 text-gray-400">
