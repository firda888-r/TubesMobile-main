...
setCat(data);
  ...
  if (loading) return <View className="text-center pt-24">Loading...</div>;
  if (!cat) return <View className="text-center pt-24">Kucing tidak ditemukan 😿</div>;

  return (
    <View className="min-h-screen bg-white pb-24">
      {/* Tombol Back & Gambar Header */}
      <View className="relative h-[40vh] md:h-[50vh] bg-gray-200">
      ...
      {/* Konten Detail */}
      <View className="px-6 py-6 -mt-6 bg-white rounded-t-3xl relative z-10 min-h-[50vh]">
        <View className="flex justify-between items-start mb-4">
          <View>
            <Text className="text-3xl font-bold text-[#2a2a2a]">{cat.name}</Text>
            <View className="flex items-center text-gray-500 mt-1">
              <MapPin size={16} className="mr-1" />
              <Text className="text-sm">Bandung, Jawa Barat</Text>
            </div>
          </div>
        ...
        <View className="grid grid-cols-3 gap-4 mb-6">
          <View className="bg-[#f7f1e8] p-3 rounded-xl text-center">
            <Text className="text-xs text-gray-500 uppercase">Usia</Text>
            <Text className="font-bold text-[#2a2a2a]">{cat.age} Tahun</Text>
          </div>
          <View className="bg-[#f7f1e8] p-3 rounded-xl text-center">
            <Text className="text-xs text-gray-500 uppercase">Gender</Text>
            <Text className="font-bold text-[#2a2a2a]">{cat.gender || 'Unknown'}</Text>
          </div>
          <View className="bg-[#f7f1e8] p-3 rounded-xl text-center">
            <Text className="text-xs text-gray-500 uppercase">Ras</Text>
            <Text className="font-bold text-[#2a2a2a] truncate">{cat.breed}</Text>
          </div>
        </div>
        ...
        <View className="mb-8">
          ...
          <Text className="text-gray-600 leading-relaxed text-sm text-justify">
            {cat.description || "Tidak ada deskripsi tersedia untuk kucing ini."}
          </Text>
        </div>

        {/* Tombol Aksi */}
        <View className="fixed bottom-20 left-0 w-full px-6 md:static md:px-0">
          <button 
            onClick={handleAdopt}
