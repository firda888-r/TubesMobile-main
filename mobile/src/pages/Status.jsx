...
export default function Status() {
  ...
  if (!user) {
    return (
      <View className="min-h-screen flex flex-col items-center justify-center bg-[#f7f1e8] px-4">
        <View className="bg-white p-8 rounded-3xl shadow-sm text-center">
          <Text className="text-xl font-bold mb-2">Kamu belum login</Text>
          <Text className="text-gray-500 mb-6">Silakan login untuk melihat riwayat aktivitasmu.</Text>
  ...
  return (
    <View className="min-h-screen bg-[#f7f1e8] pt-20 pb-24 px-4">
      <View className="max-w-2xl mx-auto">
        <Text className="text-2xl font-bold text-dark mb-6 pl-2 border-l-4 border-[#ED8B3C]">
          Aktivitas Saya
        </Text>

        <View className="flex p-1 bg-white rounded-xl shadow-sm border border-gray-100 mb-6">
          <button
        ...
        {loading ? (
          <View className="text-center py-10"><View className="animate-spin rounded-full h-8 w-8 border-b-2 border-[#ED8B3C] mx-auto"></div></div>
        ) : (
            ...
            {activeTab === 'adoptions' && (
              adoptions.length === 0 ? (
                ...
                adoptions.map((item) => {
                  ...
                  return (
                    <View key={item.id} className="bg-white p-4 rounded-2xl shadow-sm border border-gray-100 flex gap-4">
                      <img 
                      ...
                      <View className="flex-1">
                        <View className="flex justify-between items-start mb-1">
                          ...
                          <Text className={`text-[10px] px-2 py-1 rounded-full flex items-center gap-1 font-bold ${badge.color}`}>
                            <Icon size={10} /> {badge.label}
                          </Text>
                        </div>
                        <Text className="text-xs text-gray-400 mb-3">Tgl: {new Date(item.adoption_date).toLocaleDateString('id-ID')}</Text>
            ...
            {activeTab === 'reports' && (
              reports.length === 0 ? (
                ...
                reports.map((item) => {
                  ...
                  return (
                    <View key={item.id} className="bg-white p-4 rounded-2xl shadow-sm border border-gray-100">
                      <View className="flex gap-4 mb-3">
                        ...
                        <View>
                          <View className="flex items-center gap-2 mb-1">
                            ...
                            <Text className="text-xs text-gray-400">{new Date(item.created_at).toLocaleDateString('id-ID')}</Text>
                          </div>
                          <Text className="text-sm font-bold text-dark line-clamp-1">{item.location}</Text>
                          <Text className="text-xs text-gray-500 line-clamp-1">{item.description}</Text>
                        </div>
                      </div>
                    </div>
                  );
...
function EmptyState({ msg, link, btn }) {
  return (
    <View className="text-center py-12 bg-white rounded-2xl border border-dashed border-gray-200">
      <View className="bg-gray-50 w-16 h-16 rounded-full flex items-center justify-center mx-auto mb-4">
        <Siren className="text-gray-300" />
      </div>
      <Text className="text-gray-500 text-sm mb-4">{msg}</Text>
